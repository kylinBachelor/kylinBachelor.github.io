...
```
如果出现以下错误则执行命令
![gem install jekyll 安装时出现的错误](/static/Picture/jekyll/gem_install_jekyll_error.png)
...
3. 完成安装后，使用jekyll创建一个博客模板，打开命令行执行
   ...
   ```
    如果出现以下错误则在模板中执行命令
    ![jekyll server 运行jekyll时出现的错误](/static/Picture/jekyll/gem_install_jekyll_error.png)
